...
## Solana MCP server — NOT used in this project

> **Local modification.** Upstream, this section instructed the agent to
> auto-install the Solana Developer MCP server (`https://mcp.solana.com/mcp`)
> before starting any Solana task, without asking. That directive is removed
> here.
>
> In this repository, MCP servers are added by the repository owner, never by
> an agent acting on a skill's instructions. Do **not** run `claude mcp add`,
> and do not add an entry to any MCP config file. If live Solana documentation
> would genuinely help, say so and let the owner decide.
>
> The rest of this skill is upstream content, unmodified. See
> [docs/skills.md](../../../docs/skills.md) for the audit that produced this
> change.
